    ...
    try {
      ...
      if (response.ok) {
        ...
        if (project.imageUrl) {
          ...
          if (project.imageUrl.startsWith('http') || project.imageUrl.startsWith('https')) {
            setPreviewUrl(project.imageUrl);
          } else {
            // For local development, use a relative path to avoid hostname issues
            setPreviewUrl(project.imageUrl);
          }
        }
      } else {
          ...
          <form onSubmit={handleSubmit} className="glass rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <div className="mt-2 relative w-full h-40 rounded-lg overflow-hidden border border-dashed border-white/20">
                  {previewUrl ? (
                    previewUrl.startsWith('data:') ? (
                      // For data URLs (file previews)
                      <Image
                        src={previewUrl}
                        alt="Preview"
                        fill
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-background/50">
                      <p className="text-sm text-foreground/50">No image preview available</p>
                    </div>
                  )}
                </div>

                <div className="mt-2">
                  <p className="text-xs text-foreground/50">Current image path (leave empty to keep current image):</p>
                  <input
                    type="text"
                    id="imageUrl"
                    name="imageUrl"
